...
for dirdata in $(find /var/www/localhost/htdocs/openemr/sites/* -maxdepth 0 -type d ); do
    ...
    echo "<?php \$_GET['site'] = '$sitename'; ?>" > /var/www/localhost/htdocs/openemr/sites/default/TEMPsql_upgrade.php
    cat /var/www/localhost/htdocs/openemr/sql_upgrade.php >> /var/www/localhost/htdocs/openemr/sites/default/TEMPsql_upgrade.php
    sed -i "/input type='submit'/d" /var/www/localhost/htdocs/openemr/sites/default/TEMPsql_upgrade.php
    sed -i "s/!empty(\$_POST\['form_submit'\])/empty(\$_POST\['form_submit'\])/" /var/www/localhost/htdocs/openemr/sites/default/TEMPsql_upgrade.php
    sed -i "s/^[   ]*\$form_old_version[   =].*$/\$form_old_version = \"$priorOpenemrVersion\";/" /var/www/localhost/htdocs/openemr/sites/default/TEMPsql_upgrade.php
